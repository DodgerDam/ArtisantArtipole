<template>
  
  <div class="search-container">
    <div class="uk-container">
      <div class="container-items-search">
        <div class="container-picto">
          <svg class="icon">
            <use xlink:href="/assets/images/sprite.svg#artisan"></use>
          </svg>
          <p class="title-search">Je recherche un artisan...</p>
        </div>
        <form v-on:submit.prevent="onRecherche" id="search_form" autocomplete="off">
          <div class="container-select-field">
            <label class="uk-form-label sr-only" for="metiers-search-form">Métiers</label>
            <select id="metiers-search-form" class="custom-select" name="metiers" v-model="selected_metier" autocomplete="off">
              <option value="0">Métiers</option>
              <option v-for="metier in moteurs.metiers" :value="metier.id" v-bind:key="metier.id">{{metier.label}}</option>
            </select>
          </div>
         <!-- <div class="container-select-field uk-margin">
            <label class="uk-form-label sr-only" for="ville-search-form">Villes</label>
            <select id="ville-search-form" class="custom-select" name="ville" v-model="selected_ville" autocomplete="off">
              <option value="0">Villes</option>
              <option v-for="ville in moteurs.villes"  v-bind:value="[ville.id_ville, ville.lat,ville.lng]"  v-bind:key="ville.id_ville">{{ville.label}} ({{ville.code}})</option>
            </select>
          </div> -->
          <div class="container-select-field uk-margin">
            <suggestions
              v-model="query"
              :options="options"
              :onInputChange="onCountryInputChange">
              <div slot="item" slot-scope="props" >
                <strong>{{props.item}}</strong>
              </div>
            </suggestions>
          </div>
          <div class="rayon-container">
            <label class="uk-form-label" for="ville-search-form">Dans un rayon de :</label>
            <div class="uk-flex">
              <div>
                <label class="uk-form-label sr-only" for="km-search-form-01">5 km</label>
                <button v-on:click="counter5 += 1,valueRayon(5)" type="button" class="tag-rayon-button" name="rayon" id="km-search-form-01">5 km</button>
              </div>
              <div>
                <label class="uk-form-label sr-only" for="km-search-form-02">10 km</label>
                <button v-on:click="counter10 += 1,valueRayon(10)" type="button" class="tag-rayon-button" name="rayon" id="km-search-form-02">10 km</button>
              </div>
              <div>
                <label class="uk-form-label sr-only" for="km-search-form-03">50 km</label>
                <button v-on:click="counter50 += 1,valueRayon(50)" type="button" class="tag-rayon-button" name="rayon" id="km-search-form-03">50 km</button>
              </div>
            </div>
          </div>
          <button type="submit" class="submit-search-button" aria-label="Valider votre recherche"><span class="txt-search-button">Rechercher</span><span uk-search-icon></span></button>
        </form>
      </div>
      <div class="pattern"></div>
    </div>
  </div>

</template>

<script>

import Suggestions from 'v-suggestions'

  export default {
    name: "Moteur",
    data() {
      return {
        ville: '',
        lat:0,
        lng:0,
        selected_metier: 0,
        selected_ville: 0,
        rayon_de:'',
        counter5: 0,
        counter10: 0,
        counter50: 0,
        tableau_rayon:[],
        query: '',
        selectedCountry: null,
        options: {placeholder: 'Ville'}
      }
    },
    computed: {
      moteurs () {
        return this.$store.state.moteurs
      },
      communes () {
        return this.$store.state.communes
      },
      artisans(){
        return this.$store.state.artisans
      },
      isloaded(){
        return this.$store.state.isloaded
      }
    },
    watch: {
      isloaded(newValue) {
        if (newValue)
          this.valueFromGet()
      },
    },

    components:{
      Suggestions
    },
    methods: {
      
      onCountryInputChange (query) {
        if (query.trim().length <= 2) {
          return null
        }
        let ville=[]

         this.$store.state.communes.features.forEach(element => {
          ville.push(element.properties.nom)
        });
                  

        // return the matching countries as an array
        return ville.filter((country) => {
          return country.toLowerCase().includes(query.toLowerCase())
        })
      },
      onCountrySelected (item) {
        this.selectedCountry = item
      },
      onSearchItemSelected (item) {
        this.selectedSearchItem = item
      },

      // moteur recherche
      onRecherche() {
        let present_id = false
        let ville=[]

        // on stocke les villes disponible
        if(this.query!=''){
          this.$store.state.communes.features.forEach(element => {
            ville.push(element.properties.nom)
            ville.push(element.properties.code)
          });
        }
        

        if(this.selected_metier!==0){
          if (this.artisans.length > 0)
          for( let i = 0; i < this.artisans.length; i++) {
            present_id = false
            this.artisans[i]['active'] = 1
            for( let j = 0; j <  this.artisans[i].specialite.metier.length; j++) {
              if (ville.includes(this.query)){
                if(isNaN(this.query)){
                  if(this.rayon_de !== ''){
                    if (this.artisans[i].specialite.metier[j].id === this.selected_metier){
                      this.$store.state.communes.features.forEach(geo => {
                        if(geo.properties.nom==this.query){
                            present_id=this.calculRayon(geo.geometry.coordinates[0][0][1],geo.geometry.coordinates[0][0][0],this.artisans[i].coordinates.lat,this.artisans[i].coordinates.lng);
                        }
                      });
                    }
                  }else{
                    if (this.query==this.artisans[i].info_adresse[0].ville){
                      if (this.artisans[i].specialite.metier[j].id === this.selected_metier) present_id = true
                    }
                  } 
                }else{
                  if(this.rayon_de !== ''){
                    if (this.artisans[i].specialite.metier[j].id === this.selected_metier){
                      this.$store.state.communes.features.forEach(geo => {
                        if(geo.properties.nom==this.query){
                            present_id=this.calculRayon(geo.geometry.coordinates[0][0][1],geo.geometry.coordinates[0][0][0],this.artisans[i].coordinates.lat,this.artisans[i].coordinates.lng);
                        }
                      });
                    }
                  }else{
                    if (this.query==this.artisans[i].info_adresse[0].code) present_id = true
                  } 
                }  
              }else{
                 if (this.artisans[i].specialite.metier[j].id === this.selected_metier) present_id = true
              }

            }
            if(!present_id){
              this.artisans[i]['active'] = 0
            }
          }
        }else{
          if(isNaN(this.query)){
            if (this.artisans.length > 0){
              for( let i = 0; i < this.artisans.length; i++) {
                present_id = false
                this.artisans[i]['active'] = 1
                    if(ville.includes(this.query)){

                      //Filtre par ville et rayon
                      if(this.rayon_de!=''){
                        this.$store.state.communes.features.forEach(geo => {
                          if(geo.properties.nom==this.query){
                            present_id=this.calculRayon(geo.geometry.coordinates[0][0][1],geo.geometry.coordinates[0][0][0],this.artisans[i].coordinates.lat,this.artisans[i].coordinates.lng);
                          }
                        });
                      }else{
                        if (this.query==this.artisans[i].info_adresse[0].ville) present_id = true
                      }
                    }
                  
                  if(!present_id){
                        this.artisans[i]['active'] = 0
                  }
              }
            }
          }else{
            if (this.artisans.length > 0){
              for( let i = 0; i < this.artisans.length; i++) {

                present_id = false
                this.artisans[i]['active'] = 1
                    if(ville.includes(this.query)){
                      //Filtre par ville et rayon
                      if(this.rayon_de!=''){
                        this.$store.state.communes.features.forEach(geo => {
                          if(geo.properties.code==this.query){
                            present_id=this.calculRayon(geo.geometry.coordinates[0][0][1],geo.geometry.coordinates[0][0][0],this.artisans[i].coordinates.lat,this.artisans[i].coordinates.lng);
                          }
                        });
                      }else{
                        if (this.query==this.artisans[i].info_adresse[0].code) present_id = true
                      }
                    }
                  
                  if(!present_id){
                        this.artisans[i]['active'] = 0
                  }
              }
            }
          }
        }
      },
      
      valueRayon(value) {
          
        if(value==5){
          if(this.counter5 % 2 != 0){
            this.tableau_rayon.push(5);
          }else{
            const index = this.tableau_rayon.indexOf(5);
            this.tableau_rayon.splice(index, 1)
          }
        }else if(value==10){
          if(this.counter10 % 2 != 0){
            this.tableau_rayon.push(10);
          }else{
            const index = this.tableau_rayon.indexOf(10);
            this.tableau_rayon.splice(index, 1)
          }
        }else if(value==50){
          if(this.counter50 % 2 != 0){
            this.tableau_rayon.push(50);
          }else{
            const index = this.tableau_rayon.indexOf(50);
            this.tableau_rayon.splice(index, 1)
          }
        }
        
        let last_input=''
        for( let i = 0; i < this.tableau_rayon.length; i++) {
            if(this.rayon_de=''){
              last_input=this.tableau_rayon[i]
            }
            if( last_input<=this.tableau_rayon[i]){
               last_input=this.tableau_rayon[i]
            }
        }
        this.rayon_de=last_input
      },
      calculRayon(coordsVilleLatitude,coordsVilleLongitude,coordsArtisanLatitude,coordsArtisanLongitude){

          let R = this.rayon_de;
          let latitutecalcul = coordsVilleLatitude-coordsArtisanLatitude;
          let longitudecalcul = coordsVilleLongitude-coordsArtisanLongitude;
          let dLat = latitutecalcul * Math.PI / 180;
          let dLon = longitudecalcul * Math.PI / 180;
          let lat1 = coordsArtisanLatitude * Math.PI / 180;
          let lat2 = coordsVilleLatitude* Math.PI / 180  

          var a = Math.sin(dLat/2) * Math.sin(dLat/2) +
          Math.sin(dLon/2) * Math.sin(dLon/2) * Math.cos(lat1) * Math.cos(lat2); 
          var c = 2 * Math.atan2(Math.sqrt(a), Math.sqrt(1-a)); 
          var d = R * c;  
          switch (R) {
            case 5:
              if(d<=0.003 || d==0){
                return true
              }else{
                return false
              }
            case 10:
              if(d<=0.011 || d==0){
                return true
              }else{
                return false
              }
            case 50:
              if(d<=0.3 || d==0){
               return true
              }else{
                return false
              } 
            default:
        }
      },
      valueFromGet(){
            let present_id = false
            var url = new URL(window.location.href);

            if(url.searchParams.get("metier")){
              var get_metier = Number(url.searchParams.get("metier"));
              var get_ville = url.searchParams.get("ville");
              var get_rayon = Number(url.searchParams.get("rayon"));
              var lat = Number(url.searchParams.get("lat"));
              var lng = Number(url.searchParams.get("lng"));
              

              if(get_metier !=''){
                  this.selected_metier=get_metier
              }

              if(get_ville!=0 || get_ville !=''){
                  this.query=get_ville
              }

              if(get_rayon!=0 || get_rayon !=''){
                  this.rayon_de=get_rayon
              }

              if(get_metier!==0){
                if (this.artisans.length > 0)
                for( let i = 0; i < this.artisans.length; i++) {
                  present_id = false
                  this.artisans[i]['active'] = 1
                  for( let j = 0; j <  this.artisans[i].specialite.metier.length; j++) {
                    if (get_ville!=''){
                      if(isNaN(get_ville)){
                        if(get_rayon !== 0){
                          if (this.artisans[i].specialite.metier[j].id === this.selected_metier){
                            present_id=this.calculRayon(lat,lng,this.artisans[i].coordinates.lat,this.artisans[i].coordinates.lng);
                          }
                        }else{
                          if (get_ville==this.artisans[i].info_adresse[0].ville){
                            if (this.artisans[i].specialite.metier[j].id === get_metier) present_id = true
                          }
                        } 
                      }else{
                        if(get_rayon !== 0){
                          if (this.artisans[i].specialite.metier[j].id === get_metier){
                              if (get_ville==this.artisans[i].info_adresse[0].ville){
                                if (this.artisans[i].specialite.metier[j].id === get_metier) present_id = true
                              }
                          }
                        }else{
                          if (get_ville==this.artisans[i].info_adresse[0].code) present_id = true
                        } 
                      }  
                    }else{
                      if (this.artisans[i].specialite.metier[j].id === get_metier) present_id = true
                    }

                  }
                  if(!present_id){
                    this.artisans[i]['active'] = 0
                  }
                }
              }else{
                if(isNaN(get_ville)){
                  if (this.artisans.length > 0){
                    for( let i = 0; i < this.artisans.length; i++) {
                      present_id = false
                      this.artisans[i]['active'] = 1
                          if(get_ville!=''){
                            //Filtre par ville et rayon
                            if(get_rayon!==0){
                              present_id=this.calculRayon(lat,lng,this.artisans[i].coordinates.lat,this.artisans[i].coordinates.lng);
                            }else{
                              if (get_ville==this.artisans[i].info_adresse[0].ville) present_id = true
                            }
                          }
                        
                        if(!present_id){
                              this.artisans[i]['active'] = 0
                        }
                    }
                  }
                }else{
                  if (this.artisans.length > 0){
                    for( let i = 0; i < this.artisans.length; i++) {
                      present_id = false
                      this.artisans[i]['active'] = 1
                          if(get_ville!=''){
                                                  console.log(get_ville)

                            //Filtre par ville et rayon
                            if(get_rayon!==0){
                              present_id=this.calculRayon(lat,lng,this.artisans[i].coordinates.lat,this.artisans[i].coordinates.lng);
                            }else{
                              if (get_ville==this.artisans[i].info_adresse[0].code) present_id = true
                            }
                          }
                        
                        if(!present_id){
                              this.artisans[i]['active'] = 0
                        }
                    }
                  }
                }
              }
              //this.onRecherche();
            }
        }
    }
  }
</script>

